...
def main():
    train_df = pd.read_csv('./train.csv') # train_df.size: 3918366 => 1306122 samples 3min/10k samples
    ...
    dist_strategy = None # tf.contrib.distribute.MirroredStrategy(num_gpus=NUM_GPUS)
    config = tf.estimator.RunConfig(train_distribute=dist_strategy, save_checkpoints_steps=SAVE_CHECKPOINTS_STEPS)
    estimator = tf.estimator.Estimator(model_fn, model_dir=OUTPUT_DIR, config=config)
    ...
    print(f"{len(train_examples)} / {TRAIN_BATCH_SIZE} * {NUM_TRAIN_EPOCHS}")
    ...
    final_result = accuracy_score(np.array(results), test_labels)
